...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent event) {
            ...
            if (action == 0) {
                ...
                if (ItemTouchHelper.this.mSelected == null) {
                    ItemTouchHelper.RecoverAnimation animation = ItemTouchHelper.this.findAnimation(event);
                    if (animation != null) {
                        ItemTouchHelper var10000 = ItemTouchHelper.this;
                        var10000.mInitialTouchX -= animation.mX;
                        var10000 = ItemTouchHelper.this;
                        var10000.mInitialTouchY -= animation.mY;
                ...
                ItemTouchHelper.this.select((ViewHolder)null, 0);
            }
    ...
    private void destroyCallbacks() {
        ...
        for(int i = recoverAnimSize - 1; i >= 0; --i) {
            ItemTouchHelper.RecoverAnimation recoverAnimation = (ItemTouchHelper.RecoverAnimation)this.mRecoverAnimations.get(0);
        ...
        if (selected != this.mSelected || actionState != this.mActionState) {
            ...
            if (this.mSelected != null) {
                ...
                if (prevSelected.itemView.getParent() != null) {
                    ...
                    ItemTouchHelper.RecoverAnimation rv = new ItemTouchHelper.RecoverAnimation(prevSelected, animationType, prevActionState, currentTranslateX, currentTranslateY, targetTranslateX, targetTranslateY) {
    ...
    ItemTouchHelper.RecoverAnimation findAnimation(MotionEvent event) {
                ...
                if (Math.abs(this.mDx) > Math.abs(this.mDy)) {
                    if ((swipeDir = this.checkHorizontalSwipe(viewHolder, flags)) > 0) {
                        if ((originalFlags & swipeDir) == 0) {
                            return ItemTouchHelper.Callback.convertToRelativeDirection(swipeDir, ViewCompat.getLayoutDirection(this.mRecyclerView));
                        }
    ...
    public abstract static class Callback {
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected, List<ItemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for(count = 0; count < recoverAnimSize; ++count) {
                ItemTouchHelper.RecoverAnimation anim = (ItemTouchHelper.RecoverAnimation)recoverAnimationList.get(count);
                anim.update();
                count = c.save();
        ...
        void onDrawOver(Canvas c, RecyclerView parent, ViewHolder selected, List<ItemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for(int i = recoverAnimSize - 1; i >= 0; --i) {
                ItemTouchHelper.RecoverAnimation anim = (ItemTouchHelper.RecoverAnimation)recoverAnimationList.get(i);

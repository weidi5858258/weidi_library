...
public final class Api {
    ...
    // Modes
    public static final String MODE_WHITELIST = "whitelist";//白名单
    public static final String MODE_BLACKLIST = "blacklist";//黑名单
    // Messages
    //防火墙状态改变的广播
    public static final String STATUS_CHANGED_MSG = "com.googlecode.droidwall.intent.action" +
            ".STATUS_CHANGED";
    public static final String TOGGLE_REQUEST_MSG = "com.googlecode.droidwall.intent.action" +
            ".TOGGLE_REQUEST";
    //用于发送广播状态（boolean）的key值
    public static final String STATUS_EXTRA = "com.googlecode.droidwall.intent.extra.STATUS";

    // Cached applications
    public static DroidApp applications[] = null;//当前类的静态内部类
    // Do we have root access?
    private static boolean hasroot = false;//是否有root权限 （默认没有）
     ...
     * @param uidsWifi   list of selected UIDs for WIFI to allow or disallow (depending on the
     *                   working mode)
     * @param uids3g     list of selected UIDs for 2G/3G to allow or disallow (depending on the
     *                   working mode)
     * @param showErrors indicates if errors should be alerted
     */
    public static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi,
                                                 List<Integer> uids3g, boolean showErrors) {
        if (ctx == null) {
            return false;
        }
        assertBinaries(ctx, showErrors);//用于判断需要的文件是否已经复制到指定目录下
        final String ITFS_WIFI[] = {"tiwlan+", "wlan+", "eth+"};
        final String ITFS_3G[] = {"rmnet+", "pdp+", "ppp+", "uwbr+"};
        final SharedPreferences prefs = ctx.getSharedPreferences(PREFS_NAME, 0);
        //在MainActivity-selectMode()方法中保存模式 如果没有进行选择操作，默认为白名单（在onPrepareOptionsMenu(Menu menu)方法中定义）
        final boolean whitelist = false;//我测试了好几遍，在黑名单模式下比较好用
        //prefs.getString(PREF_MODE, MODE_WHITELIST).equals(MODE_WHITELIST);//是否是白名单
        final boolean blacklist = !whitelist;//不是白名单，就是黑名单
        //在toggleLogEnabled()点击事件中保存值（默认为false）是否要打印日志
        final boolean logenabled = ctx.getSharedPreferences(PREFS_NAME, 0).getBoolean
                (PREF_LOGENABLED, false);

		/*
         * 在白名单模式下：只有选中的应用才可以联网
		 * 在黑名单模式下：只有不先中的应用才可以联网
		 * 程序刚装上时，默认为：whitelist = true 
		 *                 logenabled = false
		 * 我要做的事就是：只有白名单，没有黑名单（whitelist = true）。只有选中的应用才可以联网（默认都把应用给选中），不选中的应用就不能联网了。
		 */
        ...
        try {
            ...
            if (logenabled) {
                script.append("" +
                        "# Create the log and reject rules (ignore errors on the LOG target just " +
            ...
            if (whitelist && logenabled) {
                script.append("# Allow DNS lookups on white-list for a better logging (ignore " +
                        "errors)\n");
                script.append("$IPTABLES -A droidwall -p udp --dport 53 -j RETURN\n");
            }
            script.append("# Main rules (per interface)\n");
            for (final String itf : ITFS_3G) {
                script.append("$IPTABLES -A droidwall -o ").append(itf).append(" -j droidwall-3g " +
                        "|| exit\n");
            }
            for (final String itf : ITFS_WIFI) {
                script.append("$IPTABLES -A droidwall -o ").append(itf).append(" -j " +
                        "droidwall-wifi || exit\n");
            }

            script.append("# Filtering rules\n");
            final String targetRule = (whitelist ? "RETURN" : "droidwall-reject");
            //uids3g与uidsWifi是作为参数传进来的集合
            //the index of the first occurrence of the object or -1 if the object was not found
            //集合中是否有-10这个值，如果有就返回它第一次出现的位置，如果没有就返回-1（想不通：为什么要找-10这个值，有什么用处呢？）
            //集合当中存的都是大于0的int值，应该不会有-10这个值的
            final boolean any_3g = uids3g.indexOf(SPECIAL_UID_ANY) >= 0;//猜测：any_3g = false
            final boolean any_wifi = uidsWifi.indexOf(SPECIAL_UID_ANY) >= 0;
            if (whitelist && !any_wifi) {//会执行
                // When "white listing" wifi, we need to ensure that the dhcp and wifi users are
                // allowed
                int uid = android.os.Process.getUidForName("dhcp");
                if (uid != -1) {
                    script.append("# dhcp user\n");
                    script.append("$IPTABLES -A droidwall-wifi -m owner --uid-owner ").append
                            (uid).append(" -j RETURN || exit\n");
                }
            ...
            if (any_3g) {
                if (blacklist) {
                    ...
                    script.append("$IPTABLES -A droidwall-3g -j ").append(targetRule).append(" ||" +
                            " exit\n");
                }
            } else {//执行这里
            ...
            if (any_wifi) {
                if (blacklist) {
                    /* block any application on this interface */
                    script.append("$IPTABLES -A droidwall-wifi -j ").append(targetRule).append(" " +
                            "|| exit\n");
                }
            } else {//执行这里
                /* release/block individual applications on this interface */
                for (final Integer uid : uidsWifi) {
                    script.append("$IPTABLES -A droidwall-wifi -m owner --uid-owner ").append
                            (uid).append(" -j ").append(targetRule).append(" || exit\n");
                }
            }
            ...
            //下面的代码我不需要让它执行了，给showErrors传false，那么结果就会返回true了
     ...
     * This is much faster than just calling "applyIptablesRules", since it don't need to read
     * installed applications.
     *
     * @param ctx        application context (mandatory)
     * @param showErrors indicates if errors should be alerted
     *                   保存的规则是否可用，false时为不可用，不能达到限制网络的功能；true时为正常。
     */
    ...
    public static void showLog(Context ctx) {
        try {
            ...
            while ((line = r.readLine()) != null) {
                ...
                appid = unknownUID;
                if (((start = line.indexOf("UID=")) != -1) && ((end = line.indexOf(" ", start))
                        != -1)) {
                ...
                if (((start = line.indexOf("DST=")) != -1) && ((end = line.indexOf(" ", start))
                        != -1)) {
    ...
    public static DroidApp[] getApps(Context ctx) {
        if (applications != null) {
            // return cached instance缓存实例
            return applications;
        }
        final SharedPreferences prefs = ctx.getSharedPreferences(PREFS_NAME, 0);
        // allowed application names separated by pipe '|' (persisted)
        final String savedUids_wifi = prefs.getString(PREF_WIFI_UIDS, "");//PREF_WIFI_UIDS =
        // "AllowedUidsWifi"
        final String savedUids_3g = prefs.getString(PREF_3G_UIDS, "");
        ...
        try {
            ...
            for (final ApplicationInfo apinfo : installed) {
                app = map.get(apinfo.uid);//用key去得到值
            ...
            final DroidApp special[] = {
                    new DroidApp(SPECIAL_UID_ANY, "(Any application) - Same as selecting all " +
                            "applications", false, false),
                    new DroidApp(android.os.Process.getUidForName("root"), "(root) - Applications" +
                            " running as root", false, false),
                    new DroidApp(android.os.Process.getUidForName("media"), "Media server",
                            false, false),
                    new DroidApp(android.os.Process.getUidForName("vpn"), "VPN networking",
                            false, false),
            };
     ...
     *
     * @param ctx        context
     * @param showErrors indicates（显示） if errors should be alerted
     * @return false if the binary files could not be installed---二进制文件不能被安装返回false
     */
    public static boolean assertBinaries(Context ctx, boolean showErrors) {
        ...
        try {
            ...
            if (!file.exists()) {
                copyRawFile(ctx, R.raw.iptables_g1, file, "755");//5:能够读、执行，不能写
                changed = true;
            }
            // Check iptables_n1
            ...
            if (changed) {//toast_bin_installed---DroidWall: Binary files installed!
                Toast.makeText(ctx, "DroidWall: Binary files installed!", Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
    ...
    public static void setEnabled(Context ctx, boolean enabled) {
        ...
        if (prefs.getBoolean(PREF_ENABLED, true) == enabled) {
            return;//判断保存的状态与传入的状态是否一致，如果一致就返回，如果不一致就改变状态
        }
        final Editor edit = prefs.edit();
    ...
    public static final class DroidApp {
         ...
         */
        int uid;
        //属于这个应用uid的名称 属于同一个uid的应用有的有好几个
        /**
        ...
        /**
         * Screen representation of this application全屏图像
         */
        @Override
    ...
    private static final class LogInfo {
        ...
        private HashMap<String, Integer> dstBlocked; // Number of packets blocked per destination
        // IP address

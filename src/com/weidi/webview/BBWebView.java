...
public class BBWebView extends WebView {
    ...
    protected com.weidi.webview.BBViewClient viewClient;
    ...
    private void initSettings() {
        ...
        viewClient = new com.weidi.webview.BBViewClient(_context);
        ...
        String cacheDir = _context.getApplicationContext()
                .getDir("_cache", Context.MODE_PRIVATE).getPath();
        String dbDir = _context.getApplicationContext()// Context.MODE_MULTI_PROCESS
                .getDir("_db", Context.MODE_PRIVATE).getPath();
    ...
    private void injectJavascriptInterfaces(WebView webView) {
        if (webView instanceof BBWebView) {
            injectJavascriptInterfaces();
        }
    }
    ...
    private boolean invokeJSInterfaceMethod(JsPromptResult result,
                                            String interfaceName, String methodName, Object[]
                                                    args) {
    ...
    public com.weidi.webview.BBViewClient getViewClient() {
        return viewClient;
    }
    ...
    protected void initErrorPage(final NetWorkErrorClickListener errorClickListener) {
        /*if (mErrorView == null) {
            ...
            SpannableString spannable = new SpannableString(_context.getResources().getString(R
                    .string.please_retry));
            int textSize = (int) (textView.getTextSize() * 1.2f);
            spannable.setSpan(new AbsoluteSizeSpan(textSize), 0, spannable.length(), 0);
            //			spannable.setSpan(new ForegroundColorSpan(_context.getResources().getColor
            // (R.color.creditcard_blue)), 0, spannable.length(), 0);
